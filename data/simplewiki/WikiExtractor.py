...
# August 24, 2019
# Modified by @codito to include Categories in json dumps
...
options = SimpleNamespace(
    ...
    ##
    # Whether to preserve categories
    keepCategories = True,
    ...
    def extract(self, out):
        ...
        for line in text:
            if line.lstrip().startswith('[[Category:'):
                ...
                if mCat:
                    self.categories.add(mCat.group(1))

        text = [l for l in text if not l.lstrip().startswith('[[Category:')]
...
def makeInternalLink(title, label):
    ...
    if options.keepLinks:
        ...
        if options.keepCategories and title[:colon].strip() == 'Category':
